...
def atualizar(titulo_id: int, dados: schemas.TituloUpdate, db: DB, _: CurrentUser):
    ...
    if not titulo:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Titulo não encontrado")
    if titulo.status == models.StatusTitulo.PAGO:
        raise HTTPException(status.HTTP_409_CONFLICT, "Titulo quitado não pode ser alterado")
...
def baixar(titulo_id: int, dados: schemas.BaixaIn, db: DB, _: CurrentUser):
    ...
    if titulo.status in (models.StatusTitulo.PAGO, models.StatusTitulo.CANCELADO):
        raise HTTPException(status.HTTP_409_CONFLICT, f"Titulo já está {titulo.status.value}")
...
def cancelar(titulo_id: int, db: DB, _: CurrentUser):
    ...
    if not titulo:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Titulo não encontrado")
    if titulo.status == models.StatusTitulo.PAGO:
        raise HTTPException(status.HTTP_409_CONFLICT, "Titulo quitado não pode ser cancelado")
